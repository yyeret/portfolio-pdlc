...
A safe practice instance of the flow-harness workspace contract: FlowImpact Yoga's
...
**not** software delivery, to show that the harness is about flow, not about code.
...
4. **Meta-loop.** Load `flow-harness-evolve` and probe the workflow: the rework rate out of
