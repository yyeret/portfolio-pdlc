...
RULES = [
    ...
    (2, "definition of workflow broken", "Fix the harness before steering with it — run flow_lint.py "
    ...
    (10, "flowing clean", "Run the meta-loop: flow-harness-evolve"),
]
...
def run_card(data, choice):
    ...
    if item is None:
        if rule == 2:
            ...
            lines += ["Load `flow-harness-evolve` and run one meta-loop pass against the board, the flow log, "
                      "and the loop log.", ""]
        return "\n".join(lines)
