...
```bash
python3 <REPO>/skills/flow-harness/scripts/flow_lint.py  <ws>   # contract violations
python3 <REPO>/skills/flow-harness/scripts/flow_board.py <ws>   # regenerate the projection
...
|---|---|---|---|
| 1 | A human decision is past its date | Surface it; stop piling work behind it | `flow-harness-run` (decision-brief mode) |
| 2 | Lint violations: broken graph, missing contract, unevidenced exit | Fix the harness | `flow-harness-define` / `flow-harness-scaffold` |
| 3 | An item is blocked | Clear it or escalate it to a named human with a date | `flow-harness-run` |
| 4 | An item left an evidence exit with no evidence recorded | Verify and record, or send it back | `flow-harness-run` |
| 5 | An item's exit evidence is recorded and it can move | Pull it downstream | `flow-harness-run` |
| 6 | A step is over its WIP limit | Finish, do not start | `flow-harness-run` |
| 7 | An item is aging past its step threshold | Run its step contract | `flow-harness-run` |
| 8 | Items in flow, nothing pressing | Run the most downstream unrun step | `flow-harness-run` |
| 9 | Intake is starved | Pull one new item, or report that demand dried up | `flow-harness-run` |
| 10 | Board clean and flowing | Inspect and adapt the loop | `flow-harness-evolve` |
...
| "Run it where we already work — GitHub, Jira, a dashboard" | `flow-harness-integrate` |
| Advance the flow; run a cycle; prepare a decision | `flow-harness-run` |
| "Is the loop itself any good?" / cadence review | `flow-harness-evolve` |
...
- `skills/flow-harness/references/workflow-archetypes.md` · `value-streams.md` · `measurement.md` ·
  ...
  `loop-probes.md`
- `skills/flow-harness/templates/` — workflow, step, item, measures, integrations, context pack, improvement, AGENTS.
- `skills/flow-harness/example/fiy-content-engine/` — a worked instance with seeded smells.
