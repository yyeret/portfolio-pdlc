...
A definition of workflow (from `flow-driven-define` or `flow-driven-ingest`), even a rough
one. Plus: where the work lives today, who owns the stream, and any items currently in flight.

## Workflow

1. **Create the skeleton** from `skills/flow-driven/templates/`:
...
- `skills/flow-driven/templates/` — every file this skill instantiates.
- `skills/flow-driven/references/workflow-definition.md` — the contract being satisfied.
- `skills/flow-driven/example/fiy-content-engine/` — what a filled-in workspace looks like.
- Next: `flow-driven-run` for the first cycle.
