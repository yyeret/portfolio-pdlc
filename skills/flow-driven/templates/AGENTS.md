...
This folder is a **flow workspace** run on the `flow-driven` operating system (from the
...
```bash
python3 <REPO>/skills/flow-driven/scripts/flow_lint.py .     # contract violations first
python3 <REPO>/skills/flow-driven/scripts/flow_board.py .    # regenerate the projection
