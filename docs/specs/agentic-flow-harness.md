# Spec — Agentic Flow Harness (the meta-framework)

**Status**: draft v0.1 · **Realised as**: `skills/flow-harness*` in this repo ·
**Intended end state**: a standalone meta-framework, extracted from this repo, of which
`portfolio-pdlc` becomes one instance.

This is the specification for a **flow-driven agentic loop engineering workflow harness**:
a framework that takes any value stream — a PDLC, an SDLC, a content pipeline, an AI
use-case pipeline, an operational request stream — and turns it into an engineered agentic
loop with an explicit definition of workflow, evidence-based steering, per-step delegation
and run models, measurement inside the workflow, and a meta-loop that adapts the loop.

The implementation in `skills/flow-harness/` is the first, opinionated instantiation.
...
- A runtime, daemon, scheduler, or DAG engine. Orchestration is a deterministic selector
  plus whatever agent is reading; the harness must run by hand when tooling is absent.
...
- **Realised**: `flow-harness-choose`, `references/value-streams.md`.
...
- **Realised**: `flow-harness-ingest`, `references/ingest-recipes.md`.
...
- **Realised**: `flow-harness-instrument`, `references/measurement.md`, `templates/measures.md`,
...
- **Accept**: four modes (files-only, mirror, adopt, split) chosen explicitly; a field map
  ...
  touch the network.
- **Realised**: `flow-harness-integrate`, `references/integration-adapters.md`,
...
- **Realised**: `references/step-contracts.md`, `delegate_rung`, `delegate_override`,
  `flow_board.py` flags, `flow-harness-evolve`.
...
- **Realised**: `flow-harness-scaffold`, `templates/`.
...
- **Realised**: `flow_next.py`, `flow-harness-run`.
...
- **Accept**: a probe library over four families (flow, evidence, delegation, definition);
  ...
  change-log line, and a watch period; shrinking the harness is an allowed bet.
- **Realised**: `flow-harness-evolve`, `references/loop-probes.md`, `templates/improvement.md`.
...
### R18 — An evolutionary path for the harness and for human roles
...
**What moves to the meta-framework** — `skills/flow-harness*`, `skills/flow-harness/`
...
3. Re-express `portfolio-pdlc` as a flow-harness archetype plus a thin skill layer, keeping
